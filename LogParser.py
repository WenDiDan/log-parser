...
APP_TITLE = "设备日志解析器"
APP_VERSION = "1.0.6"            # 当前版本号（与 version.txt / version.json 保持一致）
...
UPDATE_MANIFEST = r"\\192.168.250.24\软件\LogParser\version.json"
    ...
    def run(self):
        ...
        for f in self.files:
            ...
            try:
                ...
                pass
        self.out_q.put((total, mod_count, hour_count, err_count))
...
class App:
    ...
    def show_stats(self):
        ...
        def worker():
            stats = q.get()
            win.after(0, lambda: self._render_stats(win, info, canvas_frame, len(sel), stats))
        StatsWorker(sel, q).start()
        threading.Thread(target=worker, daemon=True).start()

    def _render_stats(self, win, info, canvas_frame, nfiles, stats):
        """matplotlib 绘图；matplotlib 不可用时回退文本。"""
        total, mod_count, hour_count, err_count = stats
        try:
            import matplotlib
        ...
        info.configure(text="文件数 {}    总行数 {}    异常行 {}".format(
            nfiles, total, err_total))
        ...
        ax2.plot(range(len(hours)), hvals, marker="o", color=COLORS["primary"],
                 linewidth=1.5, markersize=3)
        ax2.fill_between(range(len(hours)), hvals, alpha=0.15, color=COLORS["primary"])
        ax2.set_xticks(range(0, len(hours), step))
        ax2.set_xticklabels([hours[i] for i in range(0, len(hours), step)],
                            rotation=30, ha="right", fontsize=7)
        ax2.set_title("每小时分布（最近 48 小时）", fontsize=12)
        ax2.set_ylabel("行数")
    ...
    def _show_stats_text(self, parent, info, nfiles, stats, err=None):
        ...
        total, mod_count, hour_count, err_count = stats
        info.configure(text="文件数 {}    总行数 {}    异常行 {}".format(
            nfiles, total, sum(err_count.values())))
        ...
        lines.append("═" * 62)
        lines.append(" 文件数: {}    总行数: {}".format(nfiles, total))
        lines.append("═" * 62)
        lines.append("\n【各模块行数 / 异常行数】")
        for mod in sorted(mod_count, key=mod_count.get, reverse=True):
            e = err_count.get(mod, 0)
            flag = "  ◀ 异常!" if e else ""
            lines.append("  {:<24} {:>8}   异常 {:>5}{}".format(mod, mod_count[mod], e, flag))
        lines.append("\n【每小时分布】（最近 48 个小时）")
        hours = sorted(hour_count)[-48:]
    ...
    def _apply_config(self):
        ...
        try:
            ...
            self.update_manifest = c.get("update_manifest") or UPDATE_MANIFEST
        except Exception:
            pass
...
# ---- 远程升级：UI 相关方法 ----
    ...
    def _check_update_worker(self, manual):
        try:
            if not getattr(self, "update_manifest", UPDATE_MANIFEST):
                if manual:
                    self.root.after(0, lambda: messagebox.showinfo(
                        APP_TITLE, "未配置更新服务器地址。\n"
                        "请点击菜单「帮助 → 升级源设置…」填写 version.json 的地址。"))
                return
            data = json.loads(fetch_text(self.update_manifest))
    ...
    def _upgrade_worker(self, remote):
        ...
        try:
            ...
            exe_url = resolve_exe_url(self.update_manifest, remote.get("url", ""))
    ...
    def _set_update_source(self):
        """手动配置远程升级清单（version.json）的地址，存盘后持久化"""
        from tkinter import simpledialog
        cur = getattr(self, "update_manifest", UPDATE_MANIFEST)
        val = simpledialog.askstring(
            APP_TITLE,
            "设置远程升级清单地址（version.json）\n"
            "支持 HTTP(S) 链接或局域网 UNC 路径，例如：\n"
            r"\\192.168.*.*\LogParser\version.json",
            initialvalue=cur, parent=self.root)
        if val is None:
            return
        val = val.strip()
        if not val:
            messagebox.showwarning(APP_TITLE, "地址不能为空，已取消修改")
            return
        self.update_manifest = val
        self._save_config()
        messagebox.showinfo(
            APP_TITLE,
            "升级源已更新为：\n{}\n\n下次启动或点击「检查更新…」时生效。".format(val))
